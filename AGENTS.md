...
Release workflow (`.github/workflows/release.yml`) runs GoReleaser, then **verifies all 7 release assets exist** (6 platform archives + `checksums.txt`) before continuing. `scripts/embed-checksums.js` embeds the published checksums into `npm/package.json`, then the workflow publishes `nyx-audit-cli` through CI with SLSA provenance — never publish manually; a manual publish lacks provenance and must be unpublished and re-cut through CI.

A guard step right after checkout **refuses to re-cut an already-published npm version**: it queries `https://registry.npmjs.org/nyx-audit-cli/<version>` directly and fails fast on HTTP 200, and **fails closed on any other status** (network/registry errors must never fall through into the destructive path). Re-running a tag whose version is already on npm would otherwise let GoReleaser's `--clean` swap the hosted binaries out from under the published package's immutable checksums before `npm publish` hard-fails with `EPUBLISHCONFLICT`. The GoReleaser step keeps `continue-on-error: true` — scoped to the now-safe case where the guard proved the npm version was never published, so a 422 on pre-existing assets just means "this run's npm publish never happened" and the first run's assets plus its published `checksums.txt` remain a coherent set for the npm publish. If a release must be re-cut, cut a new patch version instead — re-tagging is only valid when the version genuinely never published (no GitHub release **and** no npm version).
...
Provider-specific CLI commands (`nyx <vendor> info|import|check|inventory`) are built dynamically from the provider's `Capabilities()` — no separate CLI wiring needed. Providers that can mutate implement the optional `providers.ACLApplier`; read-only inventory is `providers.InventoryProvider` (`nyx omada inventory` shows devices, networks, ACL scopes, clients). Extra (non-capability) subcommands cover fine-grained reads and every MCP mutation (`nyx omada plan` / `apply-acl` / `plan-port` / `apply-port-profile` / `plan-lan` / `apply-lan` / `plan-ssid` / `apply-ssid`; `nyx opnsense plan-nat` / `apply-nat` / `plan-vlan` / `apply-vlan` / `plan-filter` / `apply-filter` / `plan-unbound-override` / `apply-unbound-override` / `plan-dhcp` / `apply-dhcp`). Writes are **dry-run by default**, matching MCP. `internal/cli/parity_test.go` (`TestNamedToolCLIParity`) fails if a new `omada_*` / `opnsense_*` plan/apply tool ships without a CLI extra.
...
Provider credentials resolve in order: **flags → env vars → Windows Credential Manager (Windows only) → encrypted store** (`credentials.Overlay`). Omada uses `nyx-omada-<host>` — client ID in the entry user name, client secret in the password (`cmdkey /generic:nyx-omada-<host> /user:<client-id> /pass:<client-secret>`). OPNsense uses `nyx-opnsense-<host>` — API key in the entry user name, API secret in the password (`cmdkey /generic:nyx-opnsense-<host> /user:<api-key> /pass:<api-secret>`). Both are read by `internal/credentials/credmanager` (real `CredReadW` on Windows; "not supported on this platform" stub elsewhere). A WM entry can supply credentials but never the host. The store lives at `~/.nyx/credentials.json` (override with `NYX_CREDENTIALS_FILE`) as entry `<provider>/default`, managed by `nyx credentials set|list|remove|verify` (`internal/cli/credentials.go`, package `internal/credentials`). Store read failures are silently ignored — callers just see missing credentials.

MCP tools use the same order with **tool arguments** in place of flags (e.g. `host`/`client_id`/`client_secret` → `OMADA_*` env vars → WM `nyx-omada-<host>` → `omada/default`; `api_key`/`api_secret` → `OPNSENSE_*` → WM `nyx-opnsense-<host>` → `opnsense/default`). See the `internal/mcp` entry below and `docs/bdd/mcp-credentials.md`.
...
- `nyx credentials set|list|remove|verify` manage the encrypted store (see Credential Resolution). `set` prompts on a TTY for missing required fields (secrets via no-echo `ReadPassword`); off a TTY with missing fields it errors with `use --set key=value or run on a TTY`. `verify --live` reaches the controller or probe (`service.NewOmadaService().Info`, `service.NewOpnsenseService().Info`, `probe.Diagnose`) after the field-presence check; unknown providers skip live.
...
- `internal/mcp` — **MCP (Model Context Protocol)** stdio server (`server.go`). Tools: `discover_subnet`, `check_routes`, `check_vpn`, `verify_isolation`, `run_audit`, `load_spec`, `get_interfaces`, `ping_target`, `run_doctor`, `provider_list`, `credentials_status`, Omada (`omada_get_info/list_networks/list_acls/list_clients/inventory/import/plan/apply_acl` + NAT observation `omada_list_port_forwardings/list_one_to_one_nat/get_nat_settings/nat_facts` + port/VLAN `omada_get_uplink_info/list_switch_ports/list_lan_profiles/plan_port/apply_port_profile`) and OPNsense observation tools (`opnsense_get_info/list_interfaces/list_firewall_rules/list_clients/list_port_forward_rules/list_one_to_one_rules/list_source_nat_rules/list_aliases/get_nat/list_services/list_gateways/list_bridges/list_interface_settings/get_dnsmasq_settings/get_unbound_settings/list_unbound_overrides/get_unbound_status/get_pf_statistics/list_kernel_routes/list_kea_subnets/list_kea_reservations/list_wireguard_servers/list_wireguard_clients/get_wireguard_status/inventory` + the NAT mutation pair `opnsense_plan_nat`/`opnsense_apply_nat` + Unbound host-override `opnsense_plan_unbound_override`/`opnsense_apply_unbound_override` + DHCP `opnsense_plan_dhcp`/`opnsense_apply_dhcp`). All tools return `CheckResult`-shaped JSON consistent with CLI `--json` output. Only stdio transport is implemented; HTTP is stubbed (design: [docs/mcp-http.md](docs/mcp-http.md)). Each `tools/call` is bounded by a 5-minute timeout (`toolCallTimeout`). `credentials_status` is read-only: store completeness plus env-var presence booleans, never values and never a live controller call.
  - **CLI/MCP surface split (#61 + #95):** the CLI is the bundled user surface — every advertised capability is one `nyx <vendor> <capability>` subcommand (`info`/`import`/`check`/`inventory`). Extra (non-capability) subcommands cover fine-grained reads and **every** MCP mutation; writes default to dry-run. The MCP remains the finer-grained agent surface (per-collection observation tools plus plan/apply). `check` and (for OPNsense) `import` are still covered by composition on the MCP side. Capability parity is `TestProviderCapabilitySurfaceParity`; named mutation parity is `TestNamedToolCLIParity` (`mcpMutationCLI`).
  - Provider-tool credentials (Omada + OPNsense) resolve in order: **tool args → env vars (`OMADA_HOST`/`OMADA_CLIENT_ID`/`OMADA_CLIENT_SECRET`/`OMADA_SITE`, `OPNSENSE_HOST`/`OPNSENSE_API_KEY`/`OPNSENSE_API_SECRET`) → Windows Credential Manager (entries `nyx-omada-<host>` / `nyx-opnsense-<host>`, no-op off Windows) → encrypted store** — implemented in the `omadaOptionsFromArgs`/`opnsenseOptionsFromArgs` builders (`server.go`), which also make the `tools/list` `Required` lists credential-optional (host only; `spec` additionally for `omada_plan`). Missing everywhere keeps the actionable error pointing at the env vars, the WM hint, or `nyx credentials set`. `omada_get_info` is unauthenticated and never resolves credentials. Acceptance contract: `docs/bdd/mcp-credentials.md`.
