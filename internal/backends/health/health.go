...
func runPing(ctx context.Context, target string, count int) (string, error) {
	...
	switch runtime.GOOS {
	case "windows":
		cmd = exec.CommandContext(ctx, "ping", "-n", fmt.Sprintf("%d", count), "-w", "2000", target)
	case "darwin":
		cmd = exec.CommandContext(ctx, "ping", "-c", fmt.Sprintf("%d", count), "-W", "2000", target)
	default: // linux
		cmd = exec.CommandContext(ctx, "ping", "-c", fmt.Sprintf("%d", count), "-W", "2", target)
	}
	out, err := cmd.Output()
...
func canPing(ctx context.Context, target string, size int) bool {
	...
	case "windows":
		...
		cmd = exec.CommandContext(ctx, "ping", "-n", "1", "-f", "-l", fmt.Sprintf("%d", dataSize), target)
	case "darwin":
		// macOS: -D sets DF bit, -s sets packet size
		cmd = exec.CommandContext(ctx, "ping", "-c", "1", "-D", "-s", fmt.Sprintf("%d", dataSize), target)
	default: // linux
		// Linux: -M do sets DF bit, -s sets packet size
		cmd = exec.CommandContext(ctx, "ping", "-c", "1", "-M", "do", "-s", fmt.Sprintf("%d", dataSize), target)
	}
