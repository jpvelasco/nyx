...
func (windowsReader) Read(target string) (Cred, bool, error) {
	...
	ret, _, _ := procCredRead.Call(
		uintptr(unsafe.Pointer(&targetUTF16[0])), // #nosec G103 - fixed NUL-terminated UTF-16 target
		1,                                        // CRED_TYPE_GENERIC
		0,                                        // not an enterprise credential
		uintptr(unsafe.Pointer(&outPtr)),         // #nosec G103 - CREDENTIALW out-param
	)
	...
	defer procCredFree.Call(uintptr(unsafe.Pointer(outPtr))) // #nosec G103 - release CredReadW memory

	// CREDENTIALW mirrors the fixed C struct layout; the decode below
	// walks its pointer fields under that invariant.
	userName := readUTF16(unsafe.Pointer(outPtr.userName)) // #nosec G103 - CREDENTIALW field
	// For CRED_TYPE_GENERIC the password lives in the blob as UTF-16
	// without a trailing NUL; CredentialBlobSize is in bytes.
	blob := readUTF16(unsafe.Pointer(outPtr.credBlob), int(outPtr.credBlobSize)) // #nosec G103 - CREDENTIALW field
	return Cred{ClientID: userName, ClientSecret: blob}, true, nil
}
...
func readUTF16(p unsafe.Pointer, n ...int) string {
	...
	return syscall.UTF16ToString(unsafe.Slice((*uint16)(p), words)) // #nosec G103 - bounded CREDENTIALW decode
}
